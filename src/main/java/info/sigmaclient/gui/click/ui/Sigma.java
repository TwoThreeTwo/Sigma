...
public class Sigma extends UI {
    ...
    public void categoryPanelConstructor(CategoryPanel categoryPanel, CategoryButton categoryButton, float x, float y) {
                ...
                if (getSettings(module) != null) {
                    ...
                    if (!optionsList.isEmpty()) {
                        ...
                        for (Options option : optionsList) {
                            categoryPanel.dropdownBoxes.add(new DropdownBox(option, 0, meme, categoryPanel, module));
                            meme -= 25;
                        }
                    }
                }
        ...
        if (categoryButton.name.equalsIgnoreCase("Colors")) {
            ...
            //categoryPanel.colorPreviews.add(new ColorPreview(Client.cm.getXhairColor(), "X-Hair Color", 10, 65, categoryButton));
        }
    ...
    public void dropDownDraw(DropdownBox p0, float p2, float p3, CategoryPanel panel) {
        ...
        if (hovering) {
            RenderingUtil.rectangleBordered(p0.x + xOff, yOff, xOff + 40, yOff + 9, 0.3, Colors.getColor(0, 0), Colors.getColor(90));
        }
        Client.fm.getFont("SFR 10").drawString(p0.option.getName(), (xOff - 2), (-10 + yOff), Colors.getColor(32));
        GlStateManager.pushMatrix();
    ...
    public void colorPrewviewDraw(ColorPreview colorPreview, float x, float y) {
        ...
        RenderingUtil.rectangleBordered(x1 - 5, y1 - 5, x1 + 100, y1 + 40, 1.5, Colors.getColor(150), Colors.getColor(165));
