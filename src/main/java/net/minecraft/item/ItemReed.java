...
public class ItemReed extends Item {
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (!playerIn.func_175151_a(pos, side, stack)) {
            ...
            if (worldIn.canBlockBePlaced(field_150935_a, pos, false, side, (Entity) null, stack)) {
                IBlockState var11 = field_150935_a.onBlockPlaced(worldIn, pos, side, hitX, hitY, hitZ, 0, playerIn);

                if (worldIn.setBlockState(pos, var11, 3)) {
                    var11 = worldIn.getBlockState(pos);

                    if (var11.getBlock() == field_150935_a) {
                        ItemBlock.setTileEntityNBT(worldIn, pos, stack);
                        var11.getBlock().onBlockPlacedBy(worldIn, pos, var11, playerIn, stack);
                    }

                    worldIn.playSoundEffect(pos.getX() + 0.5F, pos.getY() + 0.5F, pos.getZ() + 0.5F, field_150935_a.stepSound.getPlaceSound(), (field_150935_a.stepSound.getVolume() + 1.0F) / 2.0F, field_150935_a.stepSound.getFrequency() * 0.8F);
                    --stack.stackSize;
                    return true;
                }
            }
