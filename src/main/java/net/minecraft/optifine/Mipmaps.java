...
public class Mipmaps {
    ...
    public Mipmaps(String iconName, int width, int height, int[] data, boolean direct) {
        ...
        this.data = data;
        this.direct = direct;
        mipmapDimensions = Mipmaps.makeMipmapDimensions(width, height, iconName);
        mipmapDatas = Mipmaps.generateMipMapData(data, width, height, mipmapDimensions);

        if (direct) {
            mipmapBuffers = Mipmaps.makeMipmapBuffers(mipmapDimensions, mipmapDatas);
        }
    }
            ...
            if (scale) {
                for (int mipX = 0; mipX < mipWidth; ++mipX) {
                    for (int mipY = 0; mipY < mipHeight; ++mipY) {
                        ...
                        int pixel = Mipmaps.alphaBlend(p1, p2, p3, p4);
