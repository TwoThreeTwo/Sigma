...
public class MathHelper {
    public static final float field_180189_a = MathHelper.sqrt_float(2.0F);
    ...
    static {
        ...
        for (i = 0; i < 4096; ++i) {
            MathHelper.SIN_TABLE_FAST[i] = (float) Math.sin((i + 0.5F) / 4096.0F * ((float) Math.PI * 2F));
        }

        for (i = 0; i < 360; i += 90) {
            MathHelper.SIN_TABLE_FAST[(int) (i * 11.377778F) & 4095] = (float) Math.sin(i * 0.017453292F);
        }
    }
    ...
    public static float cos(float p_76134_0_) {
        return MathHelper.fastMath ? MathHelper.SIN_TABLE_FAST[(int) ((p_76134_0_ + ((float) Math.PI / 2F)) * 651.8986F) & 4095] : MathHelper.SIN_TABLE[(int) (p_76134_0_ * 10430.378F + 16384.0F) & 65535];
    }
    ...
    private static int calculateLogBaseTwoDeBruijn(int p_151241_0_) {
        ...
        return MathHelper.multiplyDeBruijnBitPosition[(int) (p_151241_0_ * 125613361L >> 27) & 31];
    }
