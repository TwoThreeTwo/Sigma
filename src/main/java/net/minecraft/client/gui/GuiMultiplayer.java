...
public class GuiMultiplayer extends GuiScreen implements GuiYesNoCallback {
    ...
    public void connectToSelected() {
        ...
        if (((ServerListEntryNormal) var1).getServerData().serverIP.toLowerCase().contains("hypixel")) {
            Notifications.getManager().post("Auto Config", "Use .config hypixel for premade settings!", 5000, Notifications.Type.WARNING);
            if (!Client.getModuleManager().isEnabled(AntiBot.class)) {
                Client.getModuleManager().get(AntiBot.class).toggle();
                Notifications.getManager().post("Auto Config", "AntiBot has been enabled for your protection.", Notifications.Type.WARNING);
            }
            Module antiBot = Client.getModuleManager().get(AntiBot.class);
            if (antiBot.getSetting(AntiBot.MODE) != null && !((Options) antiBot.getSetting(AntiBot.MODE).getValue()).getSelected().equals("Hypixel")) {
                ((Options) antiBot.getSetting(AntiBot.MODE).getValue()).setSelected("Hypixel");
                Notifications.getManager().post("AntiBot", "Switched to Hypixel for your protection.", Notifications.Type.NOTIFY);
            }
        } else if (((ServerListEntryNormal) var1).getServerData().serverIP.toLowerCase().contains("mineplex")) {
            Notifications.getManager().post("Auto Config", "Use .config mineplex for premade settings!", 5000, Notifications.Type.WARNING);
            if (!Client.getModuleManager().isEnabled(AntiBot.class)) {
                Client.getModuleManager().get(AntiBot.class).toggle();
                Notifications.getManager().post("Auto Config", "AntiBot has been enabled for your protection.", Notifications.Type.WARNING);
            }
            Module antiBot = Client.getModuleManager().get(AntiBot.class);
            if (antiBot.getSetting(AntiBot.MODE) != null && !((Options) antiBot.getSetting(AntiBot.MODE).getValue()).getSelected().equals("Packet")) {
                ((Options) antiBot.getSetting(AntiBot.MODE).getValue()).setSelected("Packet");
                Notifications.getManager().post("AntiBot", "Switched to Packet for your protection.", Notifications.Type.NOTIFY);
            }
        }
