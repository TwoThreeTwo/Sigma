...
public class SoundManager {
    ...
    public void updateAllSounds() {
        ++this.playTime;
        Iterator soundEntries = this.tickableSounds.iterator();
        String nextKey;
        while (soundEntries.hasNext()) {
            ITickableSound sound = (ITickableSound) soundEntries.next();
            sound.update();
            if (sound.isDonePlaying()) {
                this.stopSound(sound);
            } else {
                nextKey = (String) this.invPlayingSounds.get(sound);
                this.sndSystem.setVolume(nextKey, this.getNormalizedVolume(sound, (SoundPoolEntry) this.playingSoundPoolEntries.get(sound), this.sndHandler.getSound(sound.getSoundLocation()).getSoundCategory()));
                this.sndSystem.setPitch(nextKey, this.getNormalizedPitch(sound, (SoundPoolEntry) this.playingSoundPoolEntries.get(sound)));
                this.sndSystem.setPosition(nextKey, sound.getXPosF(), sound.getYPosF(), sound.getZPosF());
            }
        }
        soundEntries = this.playingSounds.entrySet().iterator();
        ISound nextSound;
        try {
            //TODO: Remove this try catch. AutoMine+Speedmine seemsto cuase it to die
            while (soundEntries.hasNext()) {
                Entry nextEntry = (Entry) soundEntries.next();
                nextKey = (String) nextEntry.getKey();
                nextSound = (ISound) nextEntry.getValue();
                if (!this.sndSystem.playing(nextKey)) {
                    int var5 = ((Integer) this.playingSoundsStopTime.get(nextKey)).intValue();
                    if (var5 <= this.playTime) {
                        int var6 = nextSound.getRepeatDelay();
                        if (nextSound.canRepeat() && var6 > 0) {
                            this.delayedSounds.put(nextSound, Integer.valueOf(this.playTime + var6));
                        }
                        soundEntries.remove();
                        logger.debug(LOG_MARKER, "Removed channel {} because it\'s not playing anymore", new Object[]{nextKey});
                        ...
                        try {
                            this.categorySounds.remove(this.sndHandler.getSound(nextSound.getSoundLocation()).getSoundCategory(), nextKey);
