...
public class BlockSkull extends BlockContainer {
    ...
    protected BlockSkull() {
        ...
        setDefaultState(blockState.getBaseState().withProperty(BlockSkull.field_176418_a, EnumFacing.NORTH).withProperty(BlockSkull.field_176417_b, Boolean.valueOf(false)));
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess access, BlockPos pos) {
        switch (BlockSkull.SwitchEnumFacing.field_177063_a[((EnumFacing) access.getBlockState(pos).getValue(BlockSkull.field_176418_a)).ordinal()]) {
            case 1:
            default:
                setBlockBounds(0.25F, 0.0F, 0.25F, 0.75F, 0.5F, 0.75F);
                break;
    ...
    public IBlockState onBlockPlaced(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer) {
        return getDefaultState().withProperty(BlockSkull.field_176418_a, placer.func_174811_aO()).withProperty(BlockSkull.field_176417_b, Boolean.valueOf(false));
    }
    ...
    public void onBlockHarvested(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn) {
        if (playerIn.capabilities.isCreativeMode) {
            state = state.withProperty(BlockSkull.field_176417_b, Boolean.valueOf(true));
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
        if (!worldIn.isRemote) {
            if (!((Boolean) state.getValue(BlockSkull.field_176417_b)).booleanValue()) {
                TileEntity var4 = worldIn.getTileEntity(pos);

                if (var4 instanceof TileEntitySkull) {
                    TileEntitySkull var5 = (TileEntitySkull) var4;
                    ItemStack var6 = new ItemStack(Items.skull, 1, getDamageValue(worldIn, pos));
    ...
    public boolean func_176415_b(World worldIn, BlockPos p_176415_2_, ItemStack p_176415_3_) {
        return p_176415_3_.getMetadata() == 1 && p_176415_2_.getY() >= 2 && worldIn.getDifficulty() != EnumDifficulty.PEACEFUL && !worldIn.isRemote ? func_176414_j().func_177681_a(worldIn, p_176415_2_) != null : false;
    }
    ...
    public void func_180679_a(World worldIn, BlockPos p_180679_2_, TileEntitySkull p_180679_3_) {
        if (p_180679_3_.getSkullType() == 1 && p_180679_2_.getY() >= 2 && worldIn.getDifficulty() != EnumDifficulty.PEACEFUL && !worldIn.isRemote) {
            BlockPattern var4 = func_176416_l();
            ...
            if (var5 != null) {
                ...
                for (var6 = 0; var6 < 3; ++var6) {
                    ...
                    worldIn.setBlockState(var7.getPos(), var7.func_177509_a().withProperty(BlockSkull.field_176417_b, Boolean.valueOf(true)), 2);
                }
                ...
                var14.setLocationAndAngles(var15.getX() + 0.5D, var15.getY() + 0.55D, var15.getZ() + 0.5D, var5.func_177669_b().getAxis() == EnumFacing.Axis.X ? 0.0F : 90.0F, 0.0F);
                ...
                for (var16 = 0; var16 < 120; ++var16) {
                    worldIn.spawnParticle(EnumParticleTypes.SNOWBALL, var12.getX() + worldIn.rand.nextDouble(), var12.getY() - 2 + worldIn.rand.nextDouble() * 3.9D, var12.getZ() + worldIn.rand.nextDouble(), 0.0D, 0.0D, 0.0D, new int[0]);
                }
    ...
    public IBlockState getStateFromMeta(int meta) {
        return getDefaultState().withProperty(BlockSkull.field_176418_a, EnumFacing.getFront(meta & 7)).withProperty(BlockSkull.field_176417_b, Boolean.valueOf((meta & 8) > 0));
    }
    ...
    protected BlockPattern func_176414_j() {
        if (field_176420_N == null) {
            field_176420_N = FactoryBlockPattern.start().aisle(new String[]{"   ", "###", "~#~"}).where('#', BlockWorldState.hasState(BlockStateHelper.forBlock(Blocks.soul_sand))).where('~', BlockWorldState.hasState(BlockStateHelper.forBlock(Blocks.air))).build();
        }
    ...
    protected BlockPattern func_176416_l() {
        if (field_176421_O == null) {
            field_176421_O = FactoryBlockPattern.start().aisle(new String[]{"^^^", "###", "~#~"}).where('#', BlockWorldState.hasState(BlockStateHelper.forBlock(Blocks.soul_sand))).where('^', BlockSkull.field_176419_M).where('~', BlockWorldState.hasState(BlockStateHelper.forBlock(Blocks.air))).build();
        }
