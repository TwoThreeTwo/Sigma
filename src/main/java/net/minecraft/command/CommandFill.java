...
public class CommandFill extends CommandBase {
        ...
        if (args.length < 7) {
            ...
            if (var9 > 32768) {
                ...
                if (args.length >= 10 && var5.hasTileEntity()) {
                    String var13 = CommandBase.getChatComponentFromNthArg(sender, args, 9).getUnformattedText();
                ...
                if (var9 <= 0) {
                    ...
                    CommandBase.notifyOperators(sender, this, "commands.fill.success", new Object[]{Integer.valueOf(var9)});
                }
            } else {
                throw new CommandException("commands.fill.outOfWorld", new Object[0]);
            }
    ...
    public List addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        return args.length > 0 && args.length <= 3 ? CommandBase.func_175771_a(args, 0, pos) : (args.length > 3 && args.length <= 6 ? CommandBase.func_175771_a(args, 3, pos) : (args.length == 7 ? CommandBase.func_175762_a(args, Block.blockRegistry.getKeys()) : (args.length == 9 ? CommandBase.getListOfStringsMatchingLastWord(args, new String[]{"replace", "destroy", "keep", "hollow", "outline"}) : null)));
    }
}
