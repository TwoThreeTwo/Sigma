...
public class CommandPlaySound extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        if (args.length < 2) {
            ...
            EntityPlayerMP var5 = CommandBase.getPlayer(sender, args[var31++]);
            Vec3 var6 = sender.getPositionVector();
            double var7 = var6.xCoord;

            if (args.length > var31) {
                var7 = CommandBase.func_175761_b(var7, args[var31++], true);
            }

            double var9 = var6.yCoord;

            if (args.length > var31) {
                var9 = CommandBase.func_175769_b(var9, args[var31++], 0, 0, false);
            }

            double var11 = var6.zCoord;

            if (args.length > var31) {
                var11 = CommandBase.func_175761_b(var11, args[var31++], true);
            }

            double var13 = 1.0D;

            if (args.length > var31) {
                var13 = CommandBase.parseDouble(args[var31++], 0.0D, 3.4028234663852886E38D);
            }

            double var15 = 1.0D;

            if (args.length > var31) {
                var15 = CommandBase.parseDouble(args[var31++], 0.0D, 2.0D);
            }

            double var17 = 0.0D;

            if (args.length > var31) {
                var17 = CommandBase.parseDouble(args[var31], 0.0D, 1.0D);
            }
            ...
            CommandBase.notifyOperators(sender, this, "commands.playsound.success", new Object[]{var4, var5.getName()});
        }
    }
