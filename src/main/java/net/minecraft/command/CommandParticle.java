...
public class CommandParticle extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        if (args.length < 8) {
            ...
            if (!var3) {
                ...
                double var32 = ((float) CommandBase.func_175761_b(var31.xCoord, args[1], true));
                double var9 = ((float) CommandBase.func_175761_b(var31.yCoord, args[2], true));
                double var11 = ((float) CommandBase.func_175761_b(var31.zCoord, args[3], true));
                double var13 = ((float) CommandBase.parseDouble(args[4]));
                double var15 = ((float) CommandBase.parseDouble(args[5]));
                double var17 = ((float) CommandBase.parseDouble(args[6]));
                double var19 = ((float) CommandBase.parseDouble(args[7]));
                int var21 = 0;

                if (args.length > 8) {
                    var21 = CommandBase.parseInt(args[8], 0);
                }
                ...
                if (var23 instanceof WorldServer) {
                    ...
                    CommandBase.notifyOperators(sender, this, "commands.particle.success", new Object[]{var30, Integer.valueOf(Math.max(var21, 1))});
                }
            }
        }
    }
    ...
    public List addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        return args.length == 1 ? CommandBase.getListOfStringsMatchingLastWord(args, EnumParticleTypes.func_179349_a()) : (args.length > 1 && args.length <= 4 ? CommandBase.func_175771_a(args, 1, pos) : (args.length == 9 ? CommandBase.getListOfStringsMatchingLastWord(args, new String[]{"normal", "force"}) : null));
    }
}
