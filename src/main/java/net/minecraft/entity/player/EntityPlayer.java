...
import info.sigmaclient.event.impl.EventDeath;
    ...
    public void attackTargetEntityWithCurrentItem(Entity targetEntity) {
        if (targetEntity.canAttackWithItem()) {
            if (!targetEntity.hitByEntity(this)) {
                ...
                byte var3 = 0;
                float enchantedCrit = 0.0F;
                if (targetEntity instanceof EntityLivingBase) {
                    enchantedCrit = EnchantmentHelper.func_152377_a(this.getHeldItem(), ((EntityLivingBase) targetEntity).getCreatureAttribute());
                } else {
                    enchantedCrit = EnchantmentHelper.func_152377_a(this.getHeldItem(), EnumCreatureAttribute.UNDEFINED);
                }
                int knockback = var3 + EnchantmentHelper.getRespiration(this);
                if (this.isSprinting()) {
                    ++knockback;
                }
                if (var2 > 0.0F || enchantedCrit > 0.0F) {
                    boolean shouldCrit = this.fallDistance > 0.0F && !this.onGround && !this.isOnLadder() && !this.isInWater() && !this.isPotionActive(Potion.blindness) && this.ridingEntity == null && targetEntity instanceof EntityLivingBase;
                    if (shouldCrit && var2 > 0.0F) {
                        var2 *= 1.5F;
                    }
                    var2 += enchantedCrit;
                    boolean var6 = false;
                    int fire = EnchantmentHelper.getFireAspectModifier(this);
                    if (targetEntity instanceof EntityLivingBase && fire > 0 && !targetEntity.isBurning()) {
                        var6 = true;
                    ...
                    boolean hasDamaged = targetEntity.attackEntityFrom(DamageSource.causePlayerDamage(this), var2);
                    if (hasDamaged) {
                        if (knockback > 0) {
                            targetEntity.addVelocity((double) (-MathHelper.sin(this.rotationYaw * (float) Math.PI / 180.0F) * (float) knockback * 0.5F), 0.1D, (double) (MathHelper.cos(this.rotationYaw * (float) Math.PI / 180.0F) * (float) knockback * 0.5F));
                            //this.motionX *= 0.6D;
                            //this.motionZ *= 0.6D;
                        ...
                        Object targetEntityObject = targetEntity;
                        if (targetEntity instanceof EntityDragonPart) {
                            IEntityMultiPart var17 = ((EntityDragonPart) targetEntity).entityDragonObj;
                            if (var17 instanceof EntityLivingBase) {
                                targetEntityObject = (EntityLivingBase) var17;
                            }
                        }
                        if (currentItem != null && targetEntityObject instanceof EntityLivingBase) {
                            currentItem.hitEntity((EntityLivingBase) targetEntityObject, this);
